...
public class CardPaymentService implements PaymentService<CardPaymentModel> {
    ...
    public PaymentResponse makePayment(CardPaymentModel request) {
        ...
        try {
            isPaymentSuccessful = paymentGatewayMock.processCardPayment(request);
            if (!isPaymentSuccessful) {
                log.info("Payment failed for transaction with ref: {}", transRef);
                throw new GeneralException(PAYMENT_ERROR.getCode(), PAYMENT_ERROR.getStatus());
            }

            log.info("Payment successful for transaction with ref: {}", transRef);
